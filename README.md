...
-   ✅ Direct Order Control v2
-   ✅ ROSTER Active / Reserve Management v1
-   ✅ Valor Level Selection & Surge Readiness v1
-   ✅ Resource Color Consistency v1
-   ✅ Player Battle Row Geometry v1
-   ✅ Window Color Customization v1
-   ✅ Enemy Skills & AI Integration v1
-   ✅ Boss / Phase AI v1
-   ✅ Display-Name / Stable-Identity Audit v1
...
-   ✅ Turn queue
-   ✅ Save / Load (version-aware Save Runtime v14)
...
**Direct Order Control v2** removes the intermediate full-screen Order destination. Selecting ORDER now transfers focus straight into the four party cards already visible in MAIN MENU. Left / Right write `Game_Party` front/back row state, Confirm picks up and swaps visual formation slots, and Cancel first drops a pending swap before returning focus to the command list. Formation order remains presentation-only and does not rewrite active-party membership, mechanical turn scheduling, stats, damage, or targeting rules.
...
**Menu Screen Layout Consistency v1** extends that consistency to the entire field-menu shell. `MenuScreenLayout` owns the responsive full-screen safe inset, 8px inter-panel gap, standard 70px title bar, and shared backdrop. MAIN MENU, ROSTER, SAVE/LOAD, AREA MAP, CONFIG/CONTROLS/WINDOW COLOR, and the character-menu family now meet the same outer screen edges instead of each choosing its own margin. MAIN MENU, ROSTER, SAVE/LOAD, and AREA MAP use the same title-bar height; character screens retain their larger actor-summary header inside the same outer shell. `Scene_Menu` paints the backdrop once and its subwindows preserve it, so changing destinations no longer swaps between subtly different screen footprints/background treatments.

**Save / Load File Menu Presentation v1** uses one shared reference-inspired file browser for both commands. A compact header reads **Select a file. / FILE 01 / SAVE** (or LOAD), while three large horizontal file cards dedicate separate space to larger saved-party portrait placeholders, leader / level / location, and saved date-time / persistent **TIME** / RUNES. The location box deliberately stops short of the right metadata divider so the center and metadata regions do not visually collide. Empty files read **EMPTY** plainly. `Window_SaveSlots` remains presentation/selection only: SAVE still calls `SaveManager.save()` and LOAD still calls `SaveManager.load()` through `Scene_Menu`. Save Runtime is v14; older saves migrate forward, while play time remains additive metadata and older files safely default to a zero clock.
...
**Character Valor Arts v1** establishes one starter Art for each current actor through dedicated `initialValorArtIds`. Tyler also begins with the non-Valor **Scan** support Skill for current battle testing. Tyler's **Unbroken** is a high-power single-target physical strike; Sarah's **Rallyheart** restores 35% Max HP to all injured allies; Aboo's **Wild Arc** damages all enemies and carries a 60% base Darkness chance; G Prime's **Zero Lock** attempts to Slow all enemies. These are data records, not actor-name checks in battle code.

**Valor Arts Runtime v2** gives Valor its own canonical `data/Valor.json` database and actor-owned `valorArtIds`, separate from `Skills.json` / `skillIds`. A Valor Art is usable only while its actor is Valor Ready and spends the full gauge exactly once when committed against at least one legal target. Surge and the VALOR progression screen read the dedicated Valor runtime, while ordinary SKILL menus read only Skills. Battle effect/target resolution is still shared where the mechanics overlap, so data ownership is separate without duplicating combat math.
...
Current actors have a data-driven Max Valor of 100. Valor persists between battles and through Save Runtime v14, caps at the actor's configured maximum, and is shown in the battle HUD, Status menu, and main-menu actor cards. Fractional gain is displayed to one decimal when needed so small valid hits remain visible. A full gauge becomes ready for the actor's currently set Valor level. The player is never forced to spend it: **VALOR READY** remains visible in the HUD, Up from Attack reveals the temporary **SURGE** chip only when requested, normal commands remain available, and the gauge stays stored until a Valor Art is actually committed.

Valor Runtime v1 establishes the gauge, gain rules, persistence, ready state, and consumption API. Each actor now also owns one prepared Valor level (1-4). Changing that set level preserves earned gauge; the Valor menu lets the player inspect learned Arts and descriptions before setting a level. Battle Surge exposes only Arts from that set level and consumes the full gauge only when an Art is committed. Character Valor Arts v1 currently supplies one canonical Level-1 Art per development actor from `Valor.json`; the intended long-term shape is two Arts for Levels 1-3 and one master Art for Level 4, pending content/progression design.
...
The current Save Runtime v14 persists both equipped accessory IDs and party accessory inventory. The existing test chest can award a Power Wrist through the validated accessory event-command path.
