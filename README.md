...
**Item Menu Presentation & Inventory Navigation v1** brings ITEM up to the same full-screen menu language as the other character surfaces while keeping it a direct MAIN MENU destination. The screen reuses `Window_ActorSummary`, adds a compact ITEM metadata panel, a wrapped description strip, and reference-inspired **Use / Arrange / Key Items** tabs. Hands-on refinement makes focus deliberately hierarchical: ITEM opens with the gold cursor on **Use** only; Confirm enters the item list; confirming an item locks that item as the pending choice and moves to party-target selection; successful use returns to the item list. The pending item remains visually marked and its name / quantity / target are repeated in the metadata and description surfaces, so quantity changes are never the only feedback. Q/Esc moves back exactly one level. **Arrange** offers only the implemented orders (`Default`, `Name`, `Most`, `Least`), and applying one returns focus directly to the Arrange heading. Party target cards retain both HP and MP. **Key Items** remains presentation-ready for future `keyItem` content and states clearly when none are owned.
...
**Save / Load File Menu Presentation v1** uses one shared reference-inspired file browser for both commands. A compact header reads **Select a file. / FILE 01 / SAVE** (or LOAD), while three large horizontal file cards dedicate separate space to saved party portrait placeholders, leader / level / location, and saved date-time / RUNES. Empty files read **EMPTY** plainly. The screen intentionally presents the real save timestamp rather than labeling it as play time, because persistent play-time tracking is not yet canonical. `Window_SaveSlots` remains presentation/selection only: SAVE still calls `SaveManager.save()` and LOAD still calls `SaveManager.load()` through `Scene_Menu`, so Save Runtime v11 and its schema remain untouched.
