...
-   ✅ Save Menu Presentation & Slot Summary v1
...
**Item Menu Presentation & Inventory Navigation v1** brings ITEM up to the same full-screen menu language as the other character surfaces while keeping it a direct MAIN MENU destination. The screen reuses `Window_ActorSummary`, adds a compact ITEM metadata panel, a wrapped description strip, and reference-inspired **Use / Arrange / Key Items** tabs. **Use** keeps the active roster visible with both HP and MP so the player can pick a party target and consume field items through the existing `Game_Party.useItem()` runtime. The redundant PARTY heading was removed after hands-on review to give the four target cards enough vertical room, and focus now uses the same dim selection panel plus gold `▶` cursor language as the rest of the UI. **Arrange** truthfully offers the implemented sort orders (`Default`, `Name`, `Most`, `Least`) with a live preview rather than inventing unsupported inventory categories. **Key Items** is presentation-ready for future `keyItem` entries and shows a clear empty state when the inventory does not yet contain any.
...
**Save Menu Presentation & Slot Summary v1** replaces the old centered SAVE box with a full-screen slot browser using the same softened panel and gold-cursor language as the rest of the menu family. Each occupied slot reads existing Save Runtime v11 data for leader, level, location, timestamp, active saved party, and RUNES; empty slots stay explicit. The presentation does not add play-time metadata or change the save schema. `Window_SaveSlots` still returns only the selected slot ID and `Scene_Menu` continues to call `SaveManager.save()`. LOAD currently shares the new slot-card renderer, with its dedicated UX pass still tracked separately.
