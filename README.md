...
**Item Menu Presentation & Inventory Navigation v1** brings ITEM up to the same full-screen menu language as the other character surfaces while keeping it a direct MAIN MENU destination. The screen reuses `Window_ActorSummary`, adds a compact ITEM metadata panel and wrapped description strip, and keeps the reference-inspired **Use / Arrange / Key Items** tabs entirely above the item-side pane rather than spending actor-column height on navigation. A full-height divider now separates the actor and item regions. Hands-on refinement makes focus deliberately hierarchical: ITEM opens with the gold cursor on **Use** only; Confirm enters the item list; confirming an item locks that item as the pending choice and moves to party-target selection; successful use returns to the item list. The pending item remains visibly marked and its name / quantity / target are repeated in the metadata and description surfaces, so quantity changes are never the only feedback. Q/Esc reverses one interaction level. **Arrange** offers only the implemented orders (`Default`, `Name`, `Most`, `Least`), and applying one returns focus directly to the Arrange heading. Four active-party cards now use the reclaimed left-pane height so LV / status / HP / MP and both gauges stay inside their frames. **Key Items** remains presentation-ready for future `keyItem` content and states clearly when none are owned.
