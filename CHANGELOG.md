...
- Added Pass 114 Menu Screen Layout Consistency v1: `MenuScreenLayout` now owns the shared full-screen field-menu safe inset, inter-panel gap, standard 70px title-bar height, and backdrop. MAIN MENU, ROSTER, SAVE/LOAD, AREA MAP, CONFIG/CONTROLS/WINDOW COLOR, and the character-menu family now terminate on the same outer screen bounds; MAIN MENU, ROSTER, SAVE/LOAD, and AREA MAP also share the same title-bar height. Scene-owned menu backdrops remain visible when switching into subwindows instead of being replaced by per-window solid fills, making menu-to-menu transitions read as one continuous interface. A dedicated layout regression locks the shared shell/load order and expands the full suite to 86 passing tests.

- Added Pass 113 display-name / stable-identity audit: mutable player-facing names are no longer treated as regression-test identity across actors, merchants, maps/events, enemies, encounters, items/equipment, Magick, Skills, Enemy Skills, Valor Arts, Essences, statuses, or phase/location records. Runtime behavior continues to resolve stable IDs, keys, effect metadata, and shop types; the one discovered runtime leak in Tactical Help now derives the current scan-effect Skill name from `Skills.json` instead of spelling `Scan` into renderer copy. The full 85-suite regression set also passes against a temporary audit copy in which every data `name` field is changed, proving current mechanics and tests no longer require the development display labels.

- Added Pass 112 restorative-vs-undead runtime and reveal-on-demand Surge: battlers may now declare canonical `undead: true` identity, and shared restorative HP effects invert into direct restorative damage against those targets across Restore Magick, healing Skills/Valor Arts, and HP-restoring Items. Full-HP undead remain legal restorative targets, Item consumption still occurs only after a successful effect, and enemy AI will not accidentally heal an undead ally into damage. Valor readiness remains visible in the HUD, while the SURGE chip itself stays absent until the player deliberately presses Up from Attack; backing down to Attack hides it again. Normal Magick / Skill / Item context help now mirrors Surge's non-redundant presentation: the selector keeps the selected name while the context panel shows only the canonical description, vertically centered across one or two wrapped lines.

- Added Pass 111 battle-context refinement and Quick Map groundwork: transient battle action banners now have a wider minimum footprint, the contextual action/target panel replaces the old upper-right control legend while it is present, and Surge descriptions attach directly to the right of the selected Valor Art without repeating the Art name. Field exploration now exposes a remappable **Quick Map** action (default `M`) that routes through a small `scope`-aware map-view contract; current maps resolve to the existing AREA MAP snapshot, leaving a clean seam for a later World Map destination without replacing or duplicating Area Map discovery state.
...
- Added Area Map Foundation v1: maps may define validated landmark/exit records with proximity discovery, `Game_System` owns shared discovered-location state, MAIN MENU exposes a direct **Map** destination, and the new AREA MAP screen scales current-zone geometry while showing the player plus discovered landmarks/exits only. Map001 and Map002 provide development fixtures, and Save Runtime v12 persists discovery state with v11-and-older migration.
- Refined story-driven naming fixtures: the protagonist developer naming square is now one-shot through self-switch state, while companion fixtures demonstrate unidentified introductions by entering name selection before the recruited actor's runtime name appears in the join message.
- Added Story-Driven Naming v2: New Game now enters the map immediately with the configured protagonist shown as **Unknown** while preserving the canonical database name as the later naming default. Map001 includes a repeatable protagonist naming fixture, and the three companion recruitment fixtures now demonstrate optional post-recruit naming while keeping each companion's database name as the default.
- Added a full project-styled name-entry keyboard with upper/lowercase letter rows, Space / Delete / Select / Default side commands, 16 visible name slots, shared gold/dim selection language, direct keyboard typing, and optional actor battle-sprite portrait presentation.
