...
`DatabaseValidator` protects the engine from malformed or inconsistent loaded data. Map contracts may optionally include `menuAccess` with boolean `allowSave` / `allowLoad` flags; unsupported menu-access keys are rejected before runtime use. Map authoring structures explicitly permit `_comment` metadata at the map / geometry / event / page / conditions / command / choice / shop-good levels; `_comment` is documentation only and has no runtime meaning. Item validation also distinguishes ordinary usable items from canonical Key Items: `keyItem: true` records may be permanent or consumable and use `effect: null`, while ordinary usable items retain their existing effect validation. `Game_Party.hasKeyItem()` / `consumeKeyItem()` own the inventory-side key-item contract, and the interpreter-level `ifKeyItem` command performs validated story possession checks with optional one-copy consumption; permanent Key Items cannot be consumed by event data.
