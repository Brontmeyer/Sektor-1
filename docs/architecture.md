...
Actor display names are mutable runtime identity, not internal identifiers. `Game_Actor` retains its canonical database name as `defaultName()` and owns normalization, maximum length, rename, and reset behavior. New games route through `Scene_NameEntry` / `Window_NameEntry` before entering the map; typing replaces the selected default while Enter confirms, Backspace edits, and Escape restores the database default. The same scene can be pushed later through the validated `nameActor` event command for companion introductions. `Game_Interpreter` resolves `{actor:<id>}` tokens in dialogue text, speakers, prompts, and choice labels so authored dialogue follows the current runtime name without hard-coded protagonist strings. Save Runtime v11 already serializes actor names, so custom names persist without a schema-version change.
