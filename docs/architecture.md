...
Save Runtime v10 serializes the full `Game_Party` actor roster rather than only the legacy leader alias. Actor state includes mutable progression, HP/MP, persistent Valor, combat stats, Weapon / Armor / Accessory equipment, learned Magick, learned non-Magick Skills, save-eligible runtime statuses, persistent Essence progression, and explicit Essence slot assignments. Party state persists Runes through the legacy internal Gil field, inventory, active battle composition, and per-actor `battleRows` (`front` / `back`). v9 saves migrate to v10 with missing row state defaulted to front while preserving the exact explicit Skill ownership stored by v9; v1-v8 retain their historical starter-Valor-Art content migration. Status restoration does not re-run initial application effects; canonical derived statuses are recomputed from restored battler state. The historical `skills` array from pre-Pass-29 saves remains a migration-only alias for old Magick ownership; current Skills persist separately as `skillIds`.
