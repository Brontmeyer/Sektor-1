...
`BattleTimeManager` owns the battle-local Active Time clock and shared readiness queue. It tracks normalized Time progress for current actors and enemies, derives fill rate from Agility plus the shared Haste / Slow multiplier contract, freezes visible Time through the existing Stop/turn-progression halt flag while a private halted-status cadence still expires Stop correctly, clears defeated battlers, clamps Ready gauges, and queues battlers when they reach 100. `Scene_Battle` supplies Battle Speed-scaled delta time, then asks `BattleManager` to claim at most one ready battler for action ownership. Actors and enemies therefore use one readiness authority instead of alternating live side rounds. Time remains deliberately absent from persistent actor/save state, while `BattleManager` still owns action legality, status turn hooks, AI execution, and action completion/reset.
