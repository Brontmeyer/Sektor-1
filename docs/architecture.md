...
`Game_System` owns the canonical persistent elapsed-play clock. `SceneManager.update()` advances it once per active frame using the game-loop delta, MAIN MENU reads it for `HH:MM:SS`, and `SaveManager` stores/restores whole elapsed seconds in save metadata. Save Runtime v13 keeps that clock while adding world-discovery persistence, and older files still receive a safe zero-time default during migration. Wall-clock save timestamps remain a separate presentation fact.


`SaveManager` owns serialization, migration, validation, and restoration of persistent game progress.

Save Runtime v13 serializes the full `Game_Party` actor roster rather than only the legacy leader alias. Actor state includes mutable progression, HP/MP, persistent Valor, combat stats, Weapon / Armor / Accessory equipment, learned Magick, learned non-Magick Skills, save-eligible runtime statuses, persistent Essence progression, and explicit Essence slot assignments. Party state persists Runes through the legacy internal Gil field, inventory, active battle composition, separate visual `battleFormationActorIds`, and per-actor `battleRows` (`front` / `back`). v12 saves migrate to v13 with selected Valor Level 1 when the field is absent; v11 saves also receive an empty area-discovery state; v10 saves copy their active-party order into missing visual formation state; v9 migration still defaults missing row state to front while preserving the exact explicit Skill ownership stored by v9; v1-v8 retain their historical starter-Valor-Art content migration. Status restoration does not re-run initial application effects; canonical derived statuses are recomputed from restored battler state. The historical `skills` array from pre-Pass-29 saves remains a migration-only alias for old Magick ownership; current Skills persist separately as `skillIds`.
