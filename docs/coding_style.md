...
Actor-owned non-Magick Skill content belongs in `data/Skills.json`; enemy-only techniques belong in `data/EnemySkill.json`; Valor Arts belong in `data/Valor.json`. Compatible definitions intentionally share a narrow effect vocabulary so battle resolution can be reused without sharing ownership namespaces. Current Skill-style metadata includes `type`, `category`, `effect`, targeting/scope, `powerMultiplier` for physical damage, `healPercent` for percentage healing, and `status` chance maps for reusable status application. Current categories are `physical`, `support`, and `control`; ordinary Skill effects include `damage`, `heal`, `inflictStatus`, `valor`, and `scan`. Do not add speculative costs, cooldowns, effect fields, or categories until a runtime contract exists for them.
...
Battle HUD geometry should remain presentation-only. Use `BattleHudLayout` for stable name, command-reserve, resource-row, transient-banner, and hint regions rather than scattering screen-coordinate arithmetic through battle-state owners. Actor-row emphasis may reflect current battle state, but it must never become the authority that decides whose turn is active. Transient action/state banners belong to scene presentation state and must not replace authoritative battle logs or result data.

Reusable presentation images belong behind `UIAssetManager`. Do not hard-code UI image paths or RPG Maker sprite-sheet source rectangles in windows/scenes/renderers. Consumers should ask for named assets or manager-owned drawing primitives and must preserve a no-image/vector fallback. Import only the assets actually selected for Sektor 1; do not copy source-engine runtime/plugin code merely because it was delivered in the same archive. Tactical Help bounds belong there as well. Presentation polish may change alpha, typography, borders, fades, cursor appearance, or spacing, but it should consume existing scan/target/turn/config state rather than introduce parallel gameplay state. Banner fade timing must remain driven by the existing Battle Message Speed clock.
...
Playable actors follow the same rule: `actorId` is canonical identity and `Game_Actor.name` is mutable player-facing text. Never branch gameplay behavior on a character's current/default name as a string, and never use the current display name as a lookup key. Authored dialogue that must mention a renameable actor should use the runtime `{actor:<id>}` token so the current name is resolved at presentation time.
...
Window colors are configuration data, not per-window constants. Read/write the four-corner palette through `ConfigManager`, and let `UIAssetManager.drawPanel()` apply the shared blend. `UIResourcePalette` remains authoritative for HP / MP / Valor colors and must not inherit the configurable window tint.
