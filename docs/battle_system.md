...
`BattleRenderer` draws the battle state. Skills, Magick, and Item selectors are treated as one rendered selection-window group while the actor's main command panel remains visible as the decision anchor. Battle Context Panel v2 gives descriptive prose its own translucent two-line surface above the HUD instead of appending it to the small control-hint strip. While Magick, Skills, Item, or Surge selection is open, the panel shows the highlighted entry's name and canonical description. During target selection the same surface becomes the target readout: allies expose live HP / MP and current status summary, unscanned enemies retain unknown tactical values, and scanned enemies expose live HP / MP plus Weak / Resist / Immune categories from `BattleScanManager`. Normal Magick / Skills / Item child windows omit redundant category headings and display four rows; Surge keeps its compact prepared-level heading because that level is decision-relevant.
