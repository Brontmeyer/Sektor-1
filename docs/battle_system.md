...
Battle Command Navigation & Side Actions exposes **Escape** and **Defend** as temporary horizontal side actions rather than permanent rows. Left input from the main command list reveals the smaller Escape panel; right input reveals the smaller Defend panel. **Surge** is different: it does not exist visually until the current actor has a full Valor gauge and at least one learned Art at the actor's prepared Valor level. When ready, a gold Surge chip appears above Attack; Up from Attack focuses it, Down returns to Attack, and the ordinary Attack / Magick / Skills / Item commands remain available while Valor is stored.
