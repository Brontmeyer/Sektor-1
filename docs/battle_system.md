...
Active Time Battle uses a battle-local `0..100` Time clock for every current actor and enemy. The gauge is runtime-only and is not written to actor data or save files. Standard encounters seed every battler independently from RNG inside a `0..75` opening band. This breaks same-stat synchronization and reduces opening barrages without allowing a normal encounter to begin already Ready; future formation/advantage rules may override this standard seed explicitly.
...
ATB command authority is explicit rather than inferred from animation idleness. `BattleManager` is the only system that opens/closes ATB command ownership; legacy animation cleanup never unlocks input under active-time authority. `Scene_Battle` accepts Confirm/Cancel on the main command surface only when a living party battler is the current owner and the manager is in `TURN_COMMAND`, while `BattleManager.executeCommand()` independently rejects ownerless calls. This prevents stray Confirm input during idle TIME-filling frames from crashing the update loop after `Graphics.clear()`.
...
Config Runtime v4 exposes **ATB Mode** with `Active` and `Wait`. Active keeps all non-halted Time gauges progressing through the main command bar, deeper selectors, and target selection. Wait is selective: the main Attack / Magick / Skills / Item command bar does **not** pause Time, while Magick / Skills / Item selectors and target selection do. Wait freezes Time accumulation only; already-committed action execution, animation, effects, popups, banners, and the enemy recovery gate continue normally. Ready actions therefore remain serialized in both modes without turning Wait into a global battle pause.
...
Battle Command Navigation & Side Actions exposes **Escape** and **Defend** as temporary horizontal side actions rather than permanent rows. Left input from the main command list reveals the smaller Escape panel; right input reveals the smaller Defend panel. **Surge** is different: readiness is communicated by the actor's **VALOR READY** HUD gauge, but the gold Surge chip itself remains absent from the command silhouette until the player deliberately presses Up from Attack. That input reveals and focuses Surge above Attack; Down returns to Attack and hides the chip again, while the ordinary Attack / Magick / Skills / Item commands remain available while Valor is stored.
...
`BattleManager` interprets Attack / Magick / Skills / Item / Defend / Surge after command confirmation and owns normal Escape probability/retry rules. Skills opens only ordinary non-Valor techniques; Surge reuses the battle Skill selector in a dedicated mode containing only learned Valor Arts from the actor's prepared level. `Scene_Battle` owns navigation and the final scene handoff after a successful Escape outcome. Future commands and special character mechanics should extend these boundaries without forcing unrelated command logic into rendering classes.
...
Character Valor Arts v1 established the first canonical Valor content: **Unbroken** (Tyler, high-power single-enemy damage), **Rallyheart** (Sarah, 35% Max-HP healing to all injured allies), **Wild Arc** (Aboo, all-enemy physical damage with a 60% base Darkness chance), and **Zero Lock** (G Prime, all-enemy Slow attempt). These behaviors are expressed through the reusable effect vocabulary now owned by the dedicated Valor database.

Valor Arts Runtime v2 is the actor-owned resource specialization: canonical Arts live in `data/Valor.json`, actors learn them through `valorArtIds`, and a usable Art requires Valor Ready plus the actor's currently prepared Valor level. Valor is paid only after the action still has at least one legal target, and it is paid once per committed Art rather than once per target. A later physical miss still spends the committed gauge. Valor Arts never appear in the ordinary battle Skills selector or field SKILL menu. The dedicated VALOR menu groups learned Arts by validated `valorLevel` metadata (1-4), lets the player inspect descriptions before setting exactly one prepared level, and changing that level preserves already-earned gauge. The ready-only Surge chip opens only the Arts for that prepared level; compatible targeting/effect resolution is shared with Skills without sharing their database or ownership array. The intended content shape is two Arts for Levels 1-3 and one master Art for Level 4, while exact unlock rules remain intentionally undefined.
...
Battle Items are deliberately not ally-only. When an ordinary item omits `target` metadata, its default legal groups are both `ally` and `enemy`; the selector starts on a legal ally for convenience but can cross to enemies using the same four-direction target navigation as Magick and Skills. Optional validated `target` metadata (`self`, `ally`, `enemy`) can narrow an item's legal battlers, while battle Item scope is currently single-target. HP-restoring Items now use the shared restorative path, so targeting an `undead: true` battler converts the restoration amount into restorative damage and still consumes the Item only after that effect successfully resolves. Defeated battlers remain illegal unless a future item effect explicitly adds revival semantics.
...
`BattleRenderer` draws the battle state. Skills, Magick, and Item selectors are treated as one rendered selection-window group while the actor's main command panel remains visible as the decision anchor. Battle Context Panel v2 gives descriptive prose its own translucent two-line surface above the HUD instead of appending it to the small control-hint strip. While normal Magick, Skills, or Item selection is open, the panel shows only the highlighted entry's canonical description, vertically centered as one or two wrapped lines because the selector already owns the entry name. Surge keeps its compact prepared-level selector above Attack, but its contextual description is attached directly to the right of the selected Art and likewise does not repeat the Art name; with the two-Art Level 1-3 / one-Art Level 4 contract, the description surface tracks the selected row without consuming the full width above the HUD. During target selection the shared context surface becomes the target readout: allies expose live HP / MP and current status summary, unscanned enemies retain unknown tactical values, and scanned enemies expose live HP / MP plus Weak / Resist / Immune categories from `BattleScanManager`. While this context surface is present it replaces the former upper-right selector/target control legend rather than stacking another strip against it. Normal Magick / Skills / Item child windows omit redundant category headings and display four rows. Transient action banners retain their centered role but use a wider minimum footprint so short action names read as intentional battle callouts rather than tiny pills.
...
Current target strategies are `first`, `random`, `lowestHp`, and `lowestHpRate`. Magick and Enemy Skill actions may constrain their relative target group (`ally`, `enemy`, or `self`) and legal scope (`single` / `all`) according to the referenced canonical definition. Current condition types include `always`, `selfHpBelow`, `selfHpAbove`, `allyHpBelow`, and `allyDefeated`.

Enemy Magick uses the same `Game_Battler` runtime as actor Magick. MP payment, Silence/action restrictions, damage, healing, status payloads, Reflect, elemental rules, defeated-state handling, and target legality therefore stay shared rather than being reimplemented in AI. Enemy Skills likewise reuse the Skill-definition legality, physical-damage, percentage-healing, status-application, and target-validation paths. `BattleManager.performSkillTarget()` is the shared effect-dispatch boundary used by party Skills, Enemy Skills, and compatible Valor Arts; enemy AI only chooses the action and target set. Enemy `enemySkillId` actions resolve only against `data/EnemySkill.json`, while actor Skills and Valor Arts remain isolated in `Skills.json` and `Valor.json`.

If every configured action is unusable, AI falls back to a normal Attack when Attack itself remains legal. Confuse still overrides ordinary target preference through the shared forced-random-target contract. The Test Slime currently demonstrates weighted Attack / Goo Rush / Ember behavior and conditionally considers Mend while below half HP. Goo Rush is Enemy Skill ID 1 in `EnemySkill.json` and applies its Slow rider through the same status runtime as actor Skills.
