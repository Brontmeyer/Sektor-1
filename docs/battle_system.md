...
Actor Skill ownership is separate from Magick ownership. `initialSkillIds` seed actor knowledge and runtime `skillIds` are persisted through Save Runtime v10. The old pre-Pass-29 save field named `skills` is not current Skill ownership; it remains migration-only input for historical Magick saves.
...
Current actors define `maxValor: 100` in `Actors.json`. Valor may be fractional internally so many small hits accumulate accurately; the HUD displays whole-number progress. Gauge state is clamped between `0` and `maxValor`, persists outside battle, and is serialized by Save Runtime v10. At full gauge the actor is **Valor Ready**. Valor Arts Runtime v1 now consumes the existing full-gauge boundary through Skills while leaving gauge state and consumption ownership in `Game_Actor`.
