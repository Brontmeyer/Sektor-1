...
Enemy Magick uses the same `Game_Battler` runtime as actor Magick. MP payment, Silence/action restrictions, damage, healing, status payloads, Reflect, elemental rules, defeated-state handling, and target legality therefore stay shared rather than being reimplemented in AI. Enemy Skills likewise reuse the canonical Skill legality, physical-damage, percentage-healing, status-application, and target-validation paths. `BattleManager.performSkillTarget()` is the shared effect-dispatch boundary used by party and enemy Skills; enemy AI only chooses the action and target set. Valor Arts remain actor-owned in `data/Valor.json`; enemy `skillId` actions resolve only against `Skills.json`, so enemies cannot select actor Valor Arts while compatible effect resolution remains shared.
