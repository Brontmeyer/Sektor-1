...
**Config Menu Presentation v1** gives CONFIG the same framed Sektor 1 language as the rest of the menu family: a compact description/title header split, spacious setting rows, and the shared dim selection rectangle plus gold `▶` focus cursor. Presentation does not own setting values. `ConfigManager` remains authoritative, and Window Color / Controls continue through their dedicated editors.
