...
**Order** and **ROSTER** are deliberately different concepts. Order transfers focus into the active-party cards. Pass 59 establishes persistent front/back row choice there, expressed by portrait horizontal position and saved as party state; active-slot reordering (positions 1-4) and battle-row combat consequences remain separate later work. **ROSTER** means **Remote Organization & Strategic Team Evaluation Registry** and will own active/reserve party switching when the roster grows beyond four. ROSTER is the one main-menu destination that starts hidden and should appear only after the story explicitly unlocks the system.
