...
**Area Map** is a direct, party-independent MAIN MENU destination. It represents the current zone rather than the eventual full World Map. Canonical map data owns landmark/exit definitions and discovery radii; `Game_System` owns which location IDs have actually been discovered. Presentation may show map geometry and the player's current position, but undiscovered landmark/exit names remain hidden. This same location/discovery vocabulary is the foundation the later World Map should consume rather than duplicating discovery state.
