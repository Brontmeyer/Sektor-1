...
MAGICK, SKILL, ESSENCE, EQUIP, and STATUS establish the shared character-screen identity language: portrait placeholder (until canonical portraits exist), actor name/LV, then HP and MP stacked close to that identity block with one mini gauge beneath each value. Unused header width should remain quiet until a real contextual mechanic earns the space. ESSENCE then diverges into its own slot/catalog and Resonance progression surfaces rather than pretending to be a spell list. STATUS uses three pages inspired by classic JRPG status screens while remaining faithful to Sektor 1 data: Main presents parameters/loadout, Element presents the current `elementRate()` contract, and Effect presents active statuses plus the current `statusRate()` contract. Reference-only categories such as elemental Absorb/Halve or status Attack/Defend must not be shown unless the runtime genuinely acquires those mechanics.
...
**Skill screen grammar** deliberately reuses that same actor-summary / metadata / description / three-column-list rhythm for consistency. The field SKILL screen shows ordinary non-Valor techniques only, with Category / Effect / Target / Scope metadata. Valor Arts remain in the shared Skill runtime and battle selector until the dedicated Valor menu/progression pass gives them their own player-facing presentation. SKILL is informational/read-only in the field for now; the pass does not invent field-use rules for battle techniques.
