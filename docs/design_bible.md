...
**Save / Load file presentation** shares one file-browser language without merging the actions themselves. Three large file cards follow the approved reference hierarchy: party portraits at left, leader/level/location in the center, and truthful saved timestamp plus RUNES at right. The header identifies the currently highlighted file and whether the player is saving or loading. Persistent play time must not be fabricated from timestamps or session duration; it remains a separate future runtime feature.
