...
Field dialogue and choices belong to the same configurable window family as menus. `Window_Message` and `Window_Choice` should request semantic tintable panel roles from `UIAssetManager`, allowing Config Window Color to affect conversations without making dialogue own palette data. Choice selection follows the same dim-row + gold `▶` focus language as the menus, while typewriter timing and choice input ownership remain runtime concerns rather than presentation rules.
