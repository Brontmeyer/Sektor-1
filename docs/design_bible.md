...
MAGICK, SKILL, and ESSENCE establish the shared character-screen identity language: portrait placeholder (until canonical portraits exist), actor name/LV, then HP and MP stacked close to that identity block with one mini gauge beneath each value. Unused header width should remain quiet until a real contextual mechanic earns the space. ESSENCE then diverges into its own slot/catalog and Resonance progression surfaces rather than pretending to be a spell list.
