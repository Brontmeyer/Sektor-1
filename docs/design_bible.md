...
**Magick screen grammar** follows the approved classic-JRPG reference hierarchy without copying its exact content. The player chooses an active actor before entry. MAGICK then reserves the top area for that actor's portrait placeholder / name / Level / HP / MP and selected-Magick metadata (Category, Element, Scope, MP Needed), followed by a compact description strip and a three-column learned-Magick grid. Long lists scroll by row with contextual up/down arrows. All four directions navigate the grid and support held repeat; they do not switch actors inside this screen. Field-invalid Magick stays visible but subdued so learned knowledge is not hidden merely because it cannot be cast from the field.
