...
A full Valor gauge means the actor is **Valor Ready**. **Valor Arts Runtime v2** owns those Arts in `data/Valor.json`: a learned Valor Art requires a full gauge at the actor's prepared level and spends it once when the action is committed against at least one legal target. Character Valor Arts v1 now gives the four current actors distinct first mechanical signatures without making battle code branch on their names; future unlock/progression rules and later Arts remain open design space.
...
Skills Runtime v1 established that identity before populating the catalog. Its reusable effect vocabulary includes physical damage, percentage healing, status application, scanning, and an explicit Valor-gain effect for future ordinary techniques that intentionally manipulate the gauge. That does not imply every future Skill must fit those effects or use Valor; new effects and resource models should still be added only when approved designs need them.

**Valor Arts** are a neighboring non-Magick system rather than members of the Skills database. Canonical Arts live in `data/Valor.json`, actors own `valorArtIds`, and `Game_Actor` owns their readiness/cost behavior. Battle target/effect dispatch is shared with compatible Skill mechanics where useful, without merging their data ownership. The first canonical set is **Unbroken** (Tyler), **Rallyheart** (Sarah), **Wild Arc** (Aboo), and **Zero Lock** (G Prime). These establish mechanical signatures, not a complete final character canon.
...
Summon Magick is a future Magick category. **Skills** is the ordinary non-Magick technique namespace; **Valor Arts** have their own canonical `Valor.json` namespace.
...
Item Menu Presentation & Inventory Navigation v1 keeps **Item** as a direct main-menu destination and treats the inventory as shared party property rather than an actor-owned screen. The top-left surface is a quiet PARTY INVENTORY identity panel rather than an actor card or inventory-stat dashboard; the active-party cards persist in the lower-left pane through **Use / Arrange / Key Items** so actor identity never disappears merely because the player changed inventory tabs. The three tabs are locked to one exact grid and one shared heading-to-first-row content rhythm, while the party cards reserve a deliberate gutter before the center divider. Use follows a strict interaction ladder: opening ITEM focuses the **Use** heading only; Confirm enters the inventory list; confirming a usable item records that item as pending and then enters target selection. Successful use keeps the same item armed so it can be applied repeatedly across valid actors until the player cancels or the final copy is consumed; invalid/full targets consume nothing. Cancel reverses exactly one level. Owned Weapons / Armor / Accessories may appear in muted unavailable styling for inventory context, but they are read-only here and EQUIP remains the only equipment authority. Arrange is limited to the implemented orders (`Default`, `Name`, `Most`, `Least`): entering Arrange opens a temporary sort-order popup over the inventory preview without replacing the party pane, and applying an order closes that popup and returns focus to the Arrange heading. Key Items keeps the compact metadata panel blank while focus is still on the Key Items heading; selected-item metadata appears only after the player deliberately enters the key-item list. Focus uses the shared dim selection rectangle plus gold `▶` cursor language. Reference-only categories such as Field/Battle/Throw remain omitted until the runtime can justify them. Key Items use the same party inventory ownership but have an explicit `keyItem: true` data contract. They may be permanent or consumable, carry no ordinary field-use effect, do not appear as battle-usable items, and use the soft-rose semantic Key Item color. Story events branch through `ifKeyItem`; `consume: true` is a consumption request, but permanent Key Items still satisfy the requirement and remain owned while consumable Key Items lose exactly one copy. Development fixtures may exist on test maps to verify presentation, but production key-item content should remain story-owned.

Character-menu geometry is a shared presentation contract rather than a per-window aesthetic choice. MAGICK, SKILL, ESSENCE, EQUIP, STATUS, VALOR, and ITEM use one outer geometry helper. Their compact upper-right metadata panels also use one heading contract: a 22px semibold centered title at one canonical vertical anchor, an optional 13px secondary subtitle for page/mode identity, and shared label/value columns plus row rhythm underneath. ITEM additionally uses the party/system context-heading contract in the upper-left, while actor-scoped menus continue to use `Window_ActorSummary` there. ORDER now stays inside the MAIN MENU party-card surface rather than owning a separate character-menu layout. CONFIG / CONTROLS / WINDOW COLOR deliberately remain a separate compact Config-family header, and SAVE / LOAD / Shop retain task-specific browser or merchant headers while reusing semantic palette roles rather than arbitrary colors. EQUIP and ESSENCE intentionally meet at the same center split in their lower two-panel layouts, while other screens remain free to divide lower content according to their mechanic. Save presentation may summarize information already present in Save Runtime data, but it must not fabricate play time, silently mutate the save schema, or duplicate persistence authority outside `SaveManager`. Mature menu destinations should not reserve permanent footer space for ordinary key legends once focus and controls are visually obvious; contextual instructions remain appropriate for uncommon stateful interactions such as rebinding, RGB editing, shops, results, and dialogue progression.

Playable-character names are player-facing identity and may change; actor IDs are canonical and never change with a rename. Every actor keeps the database name as a reset/default value, while runtime UI, saves, battles, and dialogue present the mutable `Game_Actor.name`. Authored dialogue that needs a renameable character name should use `{actor:<id>}` rather than spelling the default name into prose. New Game presents the protagonist naming screen before field play begins, and the same naming contract is reusable for companions when recruitment is implemented.

That identity rule is repo-wide: player-facing names on maps, map events/merchants, encounters, enemies, items/equipment, Magick, Skills, Enemy Skills, Valor Arts, Essences, statuses, phases, and Area Map locations may be revised without changing their mechanical identity. Stable IDs/keys plus explicit metadata such as `shopType`, `effect`, `category`, and target/scope contracts carry behavior. Presentation may show the current `name`; gameplay and regression contracts must not use that label as the hidden primary key.

Field dialogue and choices belong to the same configurable window family as menus. `Window_Message` and `Window_Choice` should request semantic tintable panel roles from `UIAssetManager`, allowing Config Window Color to affect conversations without making dialogue own palette data. Choice selection follows the same dim-row + gold `▶` focus language as the menus. Dialogue continuation uses a mint-teal `▼` instead of permanent input text: blink means another dialogue page or another `text` command still follows in the current dialogue run, while the final text command and choice-owned prompts suppress it. Choice UI must not appear until the full prompt has been revealed, and the reveal press must never be reused as a choice confirmation. Long dialogue is paginated inside the message frame rather than allowed to overflow it.

Owned conventional equipment lives in party inventory, while equipped Weapon / Armor / Accessory IDs belong to actors. `Game_Party` is the physical-copy authority: a single owned copy cannot be shared by multiple actors, candidate lists expose only copies available to the selected actor, and general ITEM / merchant Sell inventory surfaces count only copies that are not currently worn by anyone. Final equip / unequip mutation still terminates at actor APIs rather than direct field mutation.
...
Enemy Actions & AI v1 lets ordinary enemies express weighted and conditional combat personalities through data, and Enemy Skills Data Separation v1 gives enemy-only techniques their own `EnemySkill.json` vocabulary. The first enemy technique, Goo Rush, is an `EnemySkill.json` record rather than bespoke slime logic. Enemy `enemySkillId` actions cannot resolve actor-owned `Skills.json` or `Valor.json` records, preserving ownership boundaries while still sharing compatible damage/healing/status execution. Boss / Phase AI v1 now adds monotonic HP-threshold phases whose only special responsibility is choosing which ordinary action pool is active. Test Slime Alpha is the first canonical example. More elaborate transition effects and encounter-specific boss mechanics remain future extensions of that foundation.
...
**Order** and **ROSTER** are deliberately different concepts. Order acts directly on the active four cards already visible in MAIN MENU: selecting Order transfers focus into those cards immediately, where left/right moves Back/Front and Confirm picks up/swaps visual formation slots. There is no intermediate Order screen. Persistent front/back row choice is expressed by portrait horizontal position and saved as party state; persistent positions 1-4 remain a visual battlefield formation order. Player Battle Row Geometry v1 carries that same row language onto the battlefield: Back stands farther from the enemy side in Normal/Back Attack, while Pincer Front steps outward toward a stable actor flank and Back remains closer to party center. Row offsets are presentation-only and are excluded from target-selection scoring and rear-damage calculation. Formation slots and rows do not grant stat, damage, range, targeting eligibility, initiative, or other combat advantages. The battler's own stats and abilities remain authoritative. **ROSTER** means **Remote Operative Selection Tactical Engagement Registry**. It owns active/reserve party switching under the four-member battle-party cap, including replacing an active operative when the party is full. The configured protagonist is locked against ordinary ROSTER reserve/replacement input; story events retain authority to remove them intentionally. It stays hidden while the protagonist is alone and appears after a supporting operative has been recruited.


**Actor-targeted menu commands** share one interaction grammar: Magick, Skill, Essence, Equip, Status, and Valor first transfer focus into the active-party cards; confirming a member opens that destination for the selected actor. Item, Config, ROSTER, Save, and the temporary development Load shortcut remain direct commands. Load returns to the game-start/file flow for release, and Esc closes the menu without a redundant Exit command. Character screens may continue cycling through the active four after entry so the player does not have to back out merely to inspect another active member.

**Field-menu screen geometry** is one visual contract. `MenuScreenLayout` owns the shared responsive safe inset, 8px panel gap, standard 70px compact title bar, and deep-neutral backdrop. MAIN MENU, ROSTER, SAVE/LOAD, AREA MAP, CONFIG/CONTROLS/WINDOW COLOR, and character-menu screens must meet the same outer bounds so moving between destinations feels like changing content inside one interface rather than opening differently sized overlays. MAIN MENU, ROSTER, SAVE/LOAD, and AREA MAP share the compact title-bar height; character menus intentionally keep their larger actor-summary header while still using the same outer shell. Subwindows opened by `Scene_Menu` preserve the scene-owned backdrop instead of painting a competing full-screen background.
...
**Area Map** is a direct, party-independent MAIN MENU destination. It represents the current zone rather than the eventual full World Map. Canonical map data owns landmark/exit definitions and discovery radii; `Game_System` owns which location IDs have actually been discovered. Presentation may show map geometry and the player's current position, but undiscovered landmark/exit names remain hidden. Field exploration also exposes a remappable **Quick Map** action, default `M`; on ordinary field maps it opens this same Area Map rather than creating a second overlay implementation. The input routes through a scope-aware map-view seam so a later overworld can send the same action to a World Map while preserving the Area Map and shared discovery vocabulary.
...
Sektor 1 also has a fixed semantic UI palette through `UIThemePalette`: **gold** is active focus, **mint-teal** is information/progression/dialogue continuation, **green/red** are positive/negative comparison, **soft violet** is Hint / Important information, **soft rose** is Key Item emphasis, **white** is primary text/value, **muted blue** is secondary text, and the deep neutral is the screen backdrop. These semantic colors should remain stable even when the player customizes window-corner colors. HP / MP / Valor remain a separate resource palette.
...
**Skill screen grammar** deliberately reuses that same actor-summary / metadata / description / three-column-list rhythm for consistency. The field SKILL screen shows ordinary non-Valor techniques only, with Category / Effect / Target / Scope metadata. Valor Arts live in their dedicated Valor runtime and Surge selector, while VALOR owns their player-facing progression presentation. SKILL is informational/read-only in the field for now; the pass does not invent field-use rules for battle techniques.

**Valor screen grammar** takes structural inspiration from classic Limit menus without importing unsupported mechanics. The shared actor-summary header remains on the left; the right header owns VALOR LEVEL, the live Valor gauge, and decision-useful context for the currently selected Art (selected Art name and its Valor level) rather than redundant learned-Art counts or a separate Ready/Building label already communicated by the gauge/value. A bounded description strip follows, then LEVEL 1-4 groups occupy a two-by-two progression field. Canonical starter Arts are Level 1 through validated `valorLevel` metadata, while empty future tiers say **No Arts learned** instead of fabricating unlock requirements. The actor's known Skill IDs remain the sole ability-ownership source; the Valor screen is a progression view over the shared Skill action engine, not a duplicate runtime. Long-term unlock rules and additional Arts remain future design work.
