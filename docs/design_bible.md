...
Field dialogue and choices belong to the same configurable window family as menus. `Window_Message` and `Window_Choice` should request semantic tintable panel roles from `UIAssetManager`, allowing Config Window Color to affect conversations without making dialogue own palette data. Choice selection follows the same dim-row + gold `▶` focus language as the menus. Dialogue continuation uses a mint-teal `▼` instead of permanent input text: blink means another dialogue page or more consecutive text from the same speaker, solid means a deliberate/end pause, and choices suppress it. Choice UI must not appear until the full prompt has been revealed, and the reveal press must never be reused as a choice confirmation. Long dialogue is paginated inside the message frame rather than allowed to overflow it.
