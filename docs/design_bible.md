...
**Order** and **ROSTER** are deliberately different concepts. Order transfers focus into the active-party cards. Pass 59 establishes persistent front/back row choice there, expressed by portrait horizontal position and saved as party state; Pass 60 adds persistent positions 1-4 as a visual battlefield formation order. Formation slots do not grant stat, damage, range, targeting, initiative, or other combat advantages. The battler's own stats and abilities remain authoritative. **ROSTER** means **Remote Organization & Strategic Team Evaluation Registry** and will own active/reserve party switching when the roster grows beyond four. ROSTER is the one main-menu destination that starts hidden and should appear only after the story explicitly unlocks the system.


**Actor-targeted menu commands** share one interaction grammar: Magick, Skill, Essence, Equip, Status, and Valor first transfer focus into the active-party cards; confirming a member opens that destination for the selected actor. Item, Option, Save, and Load remain direct commands. Character screens may continue cycling through the active four after entry so the player does not have to back out merely to inspect another active member.
