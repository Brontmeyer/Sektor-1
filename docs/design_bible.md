...
Item Menu Presentation & Inventory Navigation v1 keeps **Item** as a direct main-menu destination rather than an actor-picked command, but it now adopts the same full-screen presentation family as the other menu surfaces. The screen uses **Use / Arrange / Key Items** tabs, keeps the active party visible with both HP and MP while choosing a target for field-item use, and limits Arrange to the inventory sort behaviors the engine actually supports (`Default`, `Name`, `Most`, `Least`). Focus must use the same dim selection rectangle plus gold `▶` cursor language used throughout the project, so a selected target or active tab cannot be confused with current keyboard focus. Reference-only categories such as Field/Battle/Throw are intentionally omitted until the runtime can justify them. Key Items presentation is allowed to exist ahead of the first canonical key-item content, but an empty inventory must say so plainly instead of pretending there is hidden progression.

Character-menu geometry is a shared presentation contract rather than a per-window aesthetic choice. MAGICK, SKILL, ESSENCE, EQUIP, STATUS, VALOR, and ITEM use one outer header/metadata geometry helper; each screen remains free to divide its lower content according to its mechanic. SAVE follows the same visual family but is slot-owned rather than actor-owned. Save presentation may summarize information already present in Save Runtime data, but it must not fabricate play time, silently mutate the save schema, or duplicate persistence authority outside `SaveManager`.
