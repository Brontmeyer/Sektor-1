...
- Save / Load with version-aware multi-actor Save Runtime v13, persistent area-location discovery plus learned Skills / Valor / Runes (legacy Gil field) / accessory equipment / slot-aware Essence / party-row state, v11 discovery migration, v9 row migration, and preserved v1-v8 starter-Valor-Art migration
...
- Valor Arts Runtime with actor-owned ready-state cost payment, one prepared level, preserved gauge on level changes, and ready-only Surge selection layered onto the shared Skills action pipeline
